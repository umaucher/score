...
it is the **ONLY** way to contribute new features or to modify the scope of existing features
in the **S-CORE** project.
...
used to create a Change Request including reuqired attributes as defined in
:ref:`chm_process_change_request_attributes`.
The ISSUE may be linked to SUB-ISSUEs, if required, to manage more complex Change Requests.
Implementation of Change Requests requires at least one or more PRs linked to the ISSUE created
...
:need:`[[title]] <gd_req__change__attr_impact_description>` is identical to the description filled
of an ISSUE.

:need:`[[title]] <gd_req__change__attr_impact_safety>` is part of the description.

:need:`[[title]] <gd_req__change__attr_impact_security>` is part of the description.

:need:`[[title]] <gd_req__change__types>` is defined by labe of a ISSUE.
...
The figure below shows the workflow for the simple case of a Change Request.

An ISSUE with the label according to the Change Request type is created in status ``OPEN``.
The title of the ISSUE reflects the potential change.
The Description of the ISSUE describes the details and potential impact. For the new feature
request use the template Feature request: :ref:`chm_templates`.

The ISSUE is linked to one PR in status ``DRAFT``, which may contain the
required affected work products based on defined templates or implementation and verification
proposals.
...
The Change Request is analyzed from the :need:`Contributor <rl__contributor>`
together with the :need:`Committer <rl__committer>` and the results are documented in the ISSUE.
Unless the information is not sufficient, the related PR is kept in status ``OPEN``.

:ref:`chm_checklist` can help to verify whether the information is complete.

The implementation part of the Change Request is reviewed according the checklists of the affected
work products. Verification of the implementation must be successful.
...
:need:`Committer <rl__committer>` decides finally, if the Change Request is accepted or rejected.
...
If ``approved`` the status of the ISSUE is ``CLOSED``.


The figure below shows the workflow for a complex case of a Change Request.

The ISSUE is linked to SUB-ISSUES, and the SUB-ISSUE is linked to a PR. In principle the Change
...
Another version of a more complex Change Request case is shwon the following figure.

Here the ISSUE is linked to multiple PRs. As the PRs may accepted not at the same time, e.g. default
to priorization, the Change Request must be kept in the correct status by manually changing the
ISSUE status to ``OPEN`` unless the last linked PR is accepted.
