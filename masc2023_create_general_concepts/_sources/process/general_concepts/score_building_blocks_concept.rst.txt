...
is based on software components, which are built-up by units, the lowest element in our model.
Components are described by component requirements, a component architecture and a detailed
design.
For the technical realization of components either as library or executable BAZEL as build system
is used. As BAZEL is build upon BAZEL modules also the element (Bazel) module is introduced as
container for the realization of a component or a set of components realizing a feature.
As feature may depend on other features, thus also (Bazel) modules may depend on other (Bazel)
modules.

.. figure:: _assets/score_building_blocks_meta_model.svg
  :width: 100%
  :align: center
...
the module "kvstorage". As the build system automatically includes the dependencies, it will
also automatically integrate the module "json_al". The module "kvstorage" is built-up by the
...
Components may optionally built-up by sub-components. (Bazel) modules may also be designed
as safety element out of context (SEooC).

.. figure:: _assets/score_building_blocks_example.svg
  :width: 100%
  :align: center
