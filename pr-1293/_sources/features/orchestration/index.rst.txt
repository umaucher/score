...
Scaling across multiple Executors is achieved through instantiating additional Executor instances rather than dynamically scaling existing thread pools. Each Executor instance is independently configured regarding:
...
- All timing constraints (execution monitors) within Programs are consistently satisfied.
- Defined error-handling logic within Programs executes correctly and completely whenever timing violations or other faults occur.
- Critical tasks are executed reliably, with no instances of starvation, deadlocks, or indefinite blocking observed within Executors.
