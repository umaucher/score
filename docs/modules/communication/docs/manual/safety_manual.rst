...
.. needtable::
   ...
   for need in needs.filter_types(["aou_req"]):
      if need and "environment" in need["tags"] and need["id"].startswith("aou_req__communication"):
                results.append(need)
...
.. needtable::
   ...
   for need in needs.filter_types(["aou_req"]):
      if need and "environment" not in need["tags"] and need["id"].startswith("aou_req__communication"):
                results.append(need)
...
References
----------
| `User Manual <https://github.com/eclipse-score/communication/blob/v0.1.1/README.md>`_
| <other links>