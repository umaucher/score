...
.. workflow:: Create Problem Report
   ...
   To start review and analyis the Problem status is changed to "in review"
...
.. workflow:: Initiate and Monitor Problem Resolution
   ...
   Before closing the problem resoultion, [Comitter (rl__committer)] must check the correctness.
...
.. workflow:: Close Problem Resolution
   ...
   by the [Comitter (rl__committer)] finally.

   Otherwiser the [Comitter (rl__committer)] keeps the status "in implementation".
...
.. needtable:: RASIC Overview for Problem Resolution
   ...
   :filter: "problem_resolution" in tags and "wf" in tags
   :style: table
   :sort: status
