...
.. gd_guidl:: Quality Plan Definitions Guideline
   ...
   | Quality culture:
   | Quality as well as Safety and Security Culture is planned to grow in the SW platform. This shall be fostered by doing process compliance checks and work product reviews, as well as lessons learned after each feature development completion, using the ISO 26262-2 Table B.1 as a questionnaire and a process audit after each platform/project release.
   ...
   | Additionally, the SW platform's processes are defined with experience of several companies already performing successful safe and secure SW development. This also improves independence of reviews for the process definitions.
   |
   | Quality Management:
   | ASPICE standard is selected for quality management. Processes will always link to the :ref:`standard_iso26262` standard, :ref:`standard_isopas8926` standard, <Link to iso21434> and to the :ref:`standard_aspice` standard.
   |
   | Communication:
   | Feature development teams are interdisciplinary, so the regular (sprint) planning and review meetings enable communication (as defined in Project Management part of :need:`doc__project_mgt_plan`). Another main communication means are the Pull Request (PR) reviews.
   | Also the standard Eclipse Foundation communication strategies are used (e.g. mailing lists).
   |
   | Quality issues, non-coformances and improvements:
   ...
   | Non-conformances can also be deviations from the development process with impact on safety or security.
   ...
   | Other issues and non-conformances relevant for already delivered releases will be identified as such and communicated (as defined in Problem Resolution part of :need:`doc__project_mgt_plan`) via the :need:`wp__issue_track_system` (which is also Open Source).
   |
   ...
   | Tailoring is done for the whole SW platform by defining only the relevant processes and their resulting outcomes and an argumentation why the others are not needed in :ref:`standard_aspice`.
   |
   | **Planning quality activities:**
   | In the quality plan the nomination of the quality manager and the project manager is documented.
   ...
   | * generic planning, dealing with all work products needed only once for the platform. This is included in :need:`doc__project_quality_plan`
   |
   ...
   |
   | **Scheduling of audits, compliance checks, work product reviews, release verification and approval:**
   | Scheduling is done in the same way as for all work products definition by issues. The respective work products are :need:`wp__process_impr_report`, :need:`wp__qms_report`, :need:`wp__platform_sw_release_note` and :need:`wp__module_sw_release_note`.
   |
   | **Provision of the confidence in the use of software tools:**
   | Tool Management planning is part of the :need:`doc__project_mgt_plan`. The respective work product to be planned as an issue of the generic safety plan is the <Link WP_TOOL_EVAL>, which contains tool evaluation and, if applicable, qualification of the SW platform tool chain.
   | It is not expected that the features will have different tool chains. Tool requirements will be documented in <Link WP_TOOL_REQ>
   |
